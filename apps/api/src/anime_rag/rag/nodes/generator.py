"""Response generation node — calls LiteLLM with the prompt template."""
...
# Cost per 1M tokens (USD) — update when model pricing changes
...
def make_generator(settings: Settings):
    """Return a LangGraph node function that generates the final answer."""
    ...
    async def generate(state: RAGState) -> dict:
        ...
        context = _docs_to_context(docs[: settings.rerank_top_n])
        ...
        sources = [
            ...
            for d in docs[: settings.rerank_top_n]
        ]
